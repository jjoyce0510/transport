...
/**
 * Custom configuration properties for the {@link TransportPlugin}.
 */
public class TransportPluginConfig {

  /**
   * Supported gradle property names
   */
  ...
  public TransportPluginConfig(Project project) {
    ...
    outputDirFile = project.hasProperty(OUTPUT_DIR_PROP)
        ? new File(String.join(File.separator, project.getProjectDir().toString(), project.property("transport-plugin-output-dir").toString()))
        : project.getBuildDir(); // Build dir by default
  }

  /**
   * Retrieve a string property from a gradle project if it's been set, a default value otherwise.
   */
